            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('admin.menu-categories.update', $menuCategory) }}" method="POST">
                        @csrf
                        @method('PUT') {{-- 1. Metode untuk update --}}
                        ...
                        <div class="mb-4">
                            ...
                            <select id="restaurant_id" name="restaurant_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach ($restaurants as $restaurant)
                                    {{-- 2. Tampilkan restoran yang sudah terpilih --}}
                                    <option value="{{ $restaurant->id }}" {{ old('restaurant_id', $menuCategory->restaurant_id) == $restaurant->id ? 'selected' : '' }}>
                                        {{ $restaurant->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            {{-- 3. Tampilkan nama kategori yang ada --}}
                            <input type="text" name="name" id="name" value="{{ old('name', $menuCategory->name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            @error('name')
                                <p class="text-sm text-red-600 mt-2">{{ $message }}</p>
                            @enderror
                        </div>
